...
RENDER     render_storyboard 스크립트  코드 렌더링 (LLM 불필요)
  ↓
...
python $RD/validate_flow_prompt.py --project CH02  # MERGE 전 의무 실행 (flow_prompt 구조 검증)
python $RD/merge_records.py --project CH02 --render  # MERGE+RENDER 통합 실행
python $RD/render_storyboard.py --project CH02       # 단독 실행도 가능
...
└── README.md
```
> 스타일 변경 시 YAML 파일 1개만 수정하면 전 파이프라인(STEP 04 ANCHOR + STEP 05 flow_prompt)에 반영.
...
05_visual_direction/{RUN_ID}/{SECTION}/shot{N}.md ← STEP 05 delta (ref_images + thinking_level + flow_prompt + iv_prompt + has_human + asset_path)
...
08_storyboard/{RUN_ID}/{SECTION}/shot{N}.md       ← 스토리보드 (render_storyboard.py)
...
## NB2 flow_prompt 핵심 규칙 (v3 — 순수 한국어 자연어)

> 상세 규칙은 `.claude/agents/visual-director.md` 참조.
> visual-director, prompt-auditor 에이전트는 해당 파일의 규칙을 따른다.
> v2(구조적 태그) 프로젝트는 하위 호환 유지 — 스크립트가 자동 감지.

### 순수 한국어 서술형 포맷 (v3 간소화 — THIS {name} + style_ref)

flow_prompt는 **순수 한국어 서술형** 4단락으로 작성한다. 구도/감정/채색에만 집중.
style_ref + 턴어라운드 시트가 외형/스타일을 담당하므로 flow_prompt에서는 제거.

**API content 배열:**
```
THIS style — 이 이미지의 드로잉 스타일로 장면 전체를 그려줘:  ← style_ref (자동 첨부)
<style_ref.png>
...
flow_prompt: |           # 순수 한국어 — 구도+감정+채색만
  ...
  반드시 물레에만 deep amber 워시를 입혀줘 — 유일한 색이야.
```
...
| P4 | 채색 포인트 — "반드시 {대상}에만 {색} 워시를 입혀줘 — 유일한 색이야." |

**flow_prompt에서 제거된 항목** (ref 이미지가 담당):
...
**has_human 기반 분기:**
| has_human | 조건 | ref_images 소스 |
|-----------|------|----------------|
| `main` | 특정 캐릭터(해빛/변장) 등장 | costume ref 또는 main_turnaround.jpeg |
| `anonym` | 익명 실루엣·군중·손 | character_reference.jpeg |
| `none` | 사람 형태 없음 | 캐릭터 ref 불필요 |
...
> "prompt-auditor로 `05_visual_direction/{RUN_ID}/SECTION01/` 의 flow_prompt를 검토해줘"
