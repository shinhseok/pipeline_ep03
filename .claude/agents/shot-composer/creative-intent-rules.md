...
| `[카메라]` | 앵글·구도·프레이밍 + **캐릭터 프레임 점유율**(기본 10-15%, 최대 20%) + **구도 무게 방향** + **모션 시퀀스** (§2.5 참조) — ⚠️ CU 금지 | 필수 |
| `[조명]` | 주 광원 방향·색온도 + 바닥 그림자 암시 여부 | 필수 |
| `[감정선]` | 뉘앙스 기반 3파트: `"{nuance}. {표정/포즈} — {소품 관계}"`. 통합 연출표 7차원 톤에 맞춰 기술 | 필수 |
| `[채색]` | 채색 계획: `sempe-ink.yaml §C` 참조. **캐릭터 색 = ref 시트 (지정 불필요) + 장면 포인트 1곳(필수)**. has_human:none → 장면 포인트 최소 1곳 | 필수 |
| `[캐릭터수]` | 등장 캐릭터 수 (군중 포함). 예: `"메인 1명"`, `"메인 1명 + 보조(happy_rabbit) 1명"`, `"보조(happy_rabbit) 1명"`, `"메인 1명 + 군중 실루엣 5명"` | 필수 |
...
| 유형 | 설명 |
|------|------|
| `Motion Match` | 앞 클립의 카메라/오브젝트 이동 방향을 다음 클립이 이어받음 |
| `Contrast Cut` | 밀도·스케일·색온도 급변 |
| `Dissolve Bridge` | 앞 클립 말미 페이드 → 다음 클립 도입 페이드 (시공간 도약) |
| `Whip Transition` | 빠른 패닝 모션 블러 → 다음 장면 (에너지 전환) |
| `Scale Shock` | 인접 클립 간 크기 비율 **3배 이상** 급변 |
| `Mirror Composition` | 두 클립이 같은 구도·카메라 모션인데 **오브젝트만 다름** |
| `Subject Handoff` | 시선·물체·동선이 다음 클립으로 넘어감 |

### [이전샷] 전환 의도 기술

transition_type 뒤에 **전환 의도**를 명시한다:

```
[이전샷] Motion Match — 앞 Shot(기차 우→좌 이동)의 모션을 이어받아
  이번 Shot도 좌측으로 카메라 패닝 시작. 연속 주행감.

[이전샷] Scale Shock — 앞 Shot 기어 12%(고요) → 이번 Shot 기어 70%(압도).
  시청자가 기어의 거대함을 체감.

[이전샷] Mirror Composition — 앞 Shot 방적기 아래 아이 손 → 이번 Shot 노트북 앞 성인 손.
  구도+모션은 같고 오브젝트만 다름 — "250년 전과 지금이 같다".

[이전샷] Dissolve Bridge — 앞 Shot 여백 85% 말미 페이드아웃 → 이번 Shot 여백 15% 도입.
  시공간 도약으로 "사이에 무슨 일이 있었길래?"
```

---

## 2.5. [카메라] 모션 시퀀스 규칙 (Video-First)

모든 Shot이 비디오 클립이므로, `[카메라]` 태그에 **클립 내 카메라 모션**을 반드시 명시한다.
...
[카메라] VAST. XWS, 로우앵글. 캐릭터 8% 하단 우측.
  모션: 슬로우 틸트업(3s) → 정지+미동(2s). 하늘의 광대함 공개.

[카메라] 정면 WS. 중앙 무게.
  모션: 줌 발견 — 전체 구도(1s) → 슬로우 줌인 소품(2s). 아이러니 요소 발견.

[카메라] 측면 WS. 좌측 무게.
  모션: 수평 추적 좌→우(4s) → 정지(1s). 시간 흐름에 따른 변화 노출.
```

### 모션+감정선 연동

`[감정선]`에는 모션 아크 3단계(시작→정점→여운)를 명시한다:

| clip_rhythm | 아크 비율 (시작:정점:여운) |
|------------|------------------------|
| quick | 1:2:1 (빠른 정점, 짧은 여운) |
| standard | 1:2:2 (충분한 정점 + 여운) |
| breath | 1:2:3 (긴 여운이 전환까지 이어짐) |
...
| [보조] 나레이터 등장 | `secondary_chars: [happy_rabbit]` — 채널 공통 보조 캐릭터 |
...
video_duration: 5s
video_engine: veo3
# ⚠️ image_prompt[start], image_prompt[end]는 STEP 05 visual-director가 작성
...
creative_intent: |
  ...
  [카메라] 정면 WS→MCU 전환. 로우앵글. 모션: 줌 발견 — 전체 구도(1s) → 슬로우 줌인 나사(2s) → 정지(1s).
  [감정선] "eureka. 상체 스프링 상승, 눈 크게 떠지고 입이 벌어짐 — 한 손이 나사를 가리키며 순간 정지"
  [채색] 의상(조끼)에 muted brown 워시 1곳 + 소품(나사 스크류)에 bright accent 워시 1곳
