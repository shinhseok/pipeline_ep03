...
2. Extract per shot: `emotion_tag`, `narration_span`, `scene_type`, `duration_est`
   (Do NOT read `flow_prompt` or `has_human`)
...
> All other fields (section, local_id, duration_est, emotion_tag, narration_span, flow_prompt, has_human, etc.) → DO NOT output.
