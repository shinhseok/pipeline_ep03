...
/run-directing ─────────────────────────────────────────────────────────
  ...
  │         OUTPUT: 06_visual_direction (flow_prompt 추가된 Shot Record)
  │
  ...
  │
  └── [STEP 08] storyboard-generator/SKILL.md [gemini-3.1-flash]
        └── INPUT: 07_shot_records (Section별)
            OUTPUT: 08_storyboard (Section별 + index)
```
...
| `storyboard-generator` | gemini-3.1-flash | 07_shot_records (Section별) | 08_storyboard | YAML 렌더링 → 편집자용 스토리보드 |
...
| 08 | 스토리보드 | gemini-3.1-flash | 매우 낮음 | 창의적 결정 없는 단순 YAML 포맷 렌더링 |

**비용 최적화 원칙**:
- Opus 4.6 → 창의적 판단 + 프롬프트 엔지니어링 핵심 3단계 (STEP 04, 05, 06)
- Haiku 4.5 → 매트릭스 기반 태깅·병합 (STEP 07, Section 단위 배치 6회)
- Gemini Flash → 단순 조립·포맷 변경 (STEP 08)
- 영상 1편당 Opus 3회 + Haiku 6회 + Flash 1회 호출. STEP 07 병합으로 ~97% 비용 절감
                            ...
                            └── 05_shot_composition (shot-composer) ←── 핵심 분기점
                                    └── 06_visual_direction (visual-director)
                                            └── 07_shot_records (shot-record-builder)
                                                    └── 08_storyboard (storyboard-generator)
```
...
| `06_visual_direction` flow_prompt | Google Flow UI | Shot 단위 프롬프트 복사 → 이미지 생성 (수동) |
| `scripts/generate_images.py` | Gemini API | flow_prompt 자동 파싱 → 이미지 일괄 생성 (API) |
| `07_shot_records` bgm/sfx | CapCut | Shot별 BGM/SFX 큐 → 오디오 트랙 배치 |
| `08_storyboard` | CapCut | 편집자용 스토리보드 → 편집 가이드 |
...
|-----------|------------------------------|
| `01_research` 수정 | planner → script-director → shot-composer → visual-director → shot-record-builder → storyboard-generator |
| `02_planning` 수정 | script-director → shot-composer → visual-director → shot-record-builder → storyboard-generator |
| `04_script_final` 수정 | shot-composer → visual-director → shot-record-builder → storyboard-generator |
| `05_shot_composition` 수정 | visual-director → shot-record-builder → storyboard-generator |
| `06_visual_direction` 수정 | shot-record-builder → storyboard-generator |
| `07_shot_records` 수정 | storyboard-generator |
